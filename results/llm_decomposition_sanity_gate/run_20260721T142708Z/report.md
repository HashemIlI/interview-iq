...
- Run timestamp (UTC): 20260721T142708Z
- Model used (single model for entire run, per D77): `cohere/north-mini-code:free`
...
- **Claims produced:**
  1. الهيب هو نوع من هياكل البيانات.
  2. يعمل الهيب بنظام LIFO.
  3. في نظام LIFO، آخر شيء يتم وضعه هو أول شيء يتم إزالته.
  4. يُستخدم الهيب لتخزين استدعاءات الدوال.
  5. يُستخدم الهيب لتخزين التكرار.
...
- **Claims produced:**
  1. الفرق بين process و thread هو أن لكل process ذاكرة منفصلة عن باقي processes.
...
- **Claims produced:**
  1. يحدث الإفراط في التخصيص (overfitting) عندما يكون النموذج بسيطًا جدًا.
  2. لا يستطيع النموذج البسيط التقاط patterns في data.
  3. عدم التقاط patterns في data يؤدي إلى أداء ضعيف.
...
- **Claims produced:**
  1. عندما نقوم بتقييم model classification، ننظر إلى precision.
  2. الدقة هي عدد التوقعات الصحيحة من إجمالي التوقعات التي قال model إنها positive.
  3. هذا المقياس هو الأكثر أهمية بالنسبة لي لمعرفة ما إذا كان model يعمل بشكل جيد أم لا.
...
- **Claims produced:**
  1. Normalization يحول البيانات بحيث يكون المتوسط صفرًا.
  2. Normalization يحول البيانات بحيث يكون الانحراف المعياري واحدًا.
  3. Standardization يحول القيم إلى نطاق بين صفر وواحد.
...
- **Claims produced:**
  1. في لغة SQL، عند استخدام INNER JOIN بين جدولين، فإنه يعيد فقط الصفوف التي يوجد تطابق بينها في العمود المستخدم للجذب.
  2. إذا كان لديك جدول Orders وجدول Customers، فإن INNER JOIN سيعيد فقط الطلبات التي يوجد لها عميل مطابق موجود بالفعل.
...
- **Claims produced:**
  1. الفرق بين merge و rebase في Git هو أن rebase يدمج الفروع دون تغيير سجل الالتزامات.
  2. rebase أكثر أمانًا دائمًا من merge.
  3. لا حاجة للحديث عن النزاعات لأن Git يحلها تلقائيًا.
...
- **Claims produced:**
  ...
  2. HTTPS يستخدم نفس المنفذ port 80.
  3. HTTPS يعمل بنفس الطريقة بالضبط دون أي تشفير إضافي.
