...
## D109 — NLI module closed for the thesis: zero-shot is final; D89-b not exercised (2026-07-30)

**Decision:** The NLI scoring module is CLOSED for this thesis. The runtime arm is the zero-shot base model (MoritzLaurer/mDeBERTa-v3-base-mnli-xnli) for both the Precision and Coverage channels, as established in D89 and re-confirmed on independent evidence in D108. D89-b (retraining the LoRA adapter on labelled pairs drawn from real pipeline decompositions) is UNBLOCKED as of D106 but is DELIBERATELY NOT EXERCISED, by Ahmed's decision, under time constraints ahead of the defence. This is a decision not to run an available experiment, not a claim that the experiment is unnecessary.

**Basis for closing:** D108 re-derived the comparison on the production pipeline with a different decomposition model, prompt v4 and the glossary applied, 25 of 25 questions processed: zero-shot mean Coverage 0.2928 versus adapter 0.1211, zero-shot higher on 23 of 25. Together with D88's Precision-channel measurement (near-verbatim entailment 0.985 zero-shot versus 0.191 adapter) the direction is established on both channels.

**Design gap recorded rather than filled:** the phrase "structurally-labeled pairs" in D89's reopening condition was never given a concrete definition anywhere in this record. A verification pass this session confirmed it appears only in D89's forward-looking sentence. Any future execution of D89-b must first define its label source, and the analysis performed this session establishes that only human judgment is a valid source for entailment and contradiction labels on real claims: labels taken from the NLI model's own output would be self-distillation and would entrench F3, and a structural rule of the form "claim from the same question implies entailment" is invalid because a candidate's answer may be factually wrong, so such a rule would teach the model to entail indiscriminately and destroy contradiction detection. Neutral labels alone can be derived structurally, by pairing a claim with a chunk from a different question. Recorded so that a future attempt does not rediscover this.

**Costs of this decision, stated explicitly and required in the thesis Limitations section:**
1. Weaker contradiction detection. D89 recorded the adapter scoring a factually wrong Stack answer at -44.73 where zero-shot scored it +4.08. The same weakness was observed on real audio in D94 and D104: GN-040's deliberately wrong claim ("a Byte is 16 bits") was judged NEUTRAL and therefore not penalised at all, and the wrong answer consequently received a higher Precision value (0.8) than the correct answer (0.501). The system as configured can score a wrong answer above a right one on the Precision channel. This must be stated in the thesis, not left for the examiner to find.
2. Low Coverage in absolute terms. Mean 0.2928 over 25 correct-answer questions (D108), with floor cases below 0.01. The cause is not established; the glossary was measured as not being the cause (+0.004, D108).
3. F3 remains open. A valid Arabic paraphrase of the core TDD definition was judged CONTRADICTED at max_c = 0.994 against its own reference chunk (D88, field-confirmed D94/D104). This is the single defect most likely responsible for cost 1 and cost 2 and it is unexplained.
4. The adapter's demonstrated benefits are given up: the subject-blindness fix verified on the Gold Set at 48 of 48 (Phase 5), and its contradiction-detection advantage.

**What this decision does NOT do:** it does not retract D89's reopening condition, which remains binding and unexercised; it does not claim zero-shot is superior in all respects, which D108 and the costs above contradict; and it does not close F3, T1, F4-F8, G1 or G4.

**Thesis framing (registered so the write-up and the defence are consistent):** the fine-tuned adapter is reported as a negative result with a diagnosed mechanism — distribution mismatch caused by HARD_POS twin construction — accompanied by its pre-registered reopening condition. The generalisable finding is that a validation score computed on a model's own training distribution is not evidence of field performance: the adapter reached 0.861 macro-F1 on its own validation set and was worse than no fine-tuning at all on real pipeline data.

**Open items after this entry (unchanged in substance, restated for the record):** F3 (false contradiction, highest priority for any future work), low Coverage (D108), G4 threshold calibration — all reported scores in this thesis are computed with PRE-CALIBRATION DEFAULT thresholds (tau_E=0.9, tau=0.5, alpha=0, k=10) which have never been calibrated against human judgment, G1 (the 150 training pairs never received per-pair human review, RISK ACCEPTED in D33), T1, F4-F8, Q2, Q7, G3, Q10.

## D110 — F3 root cause found: contradiction aggregated from an unrelated chunk; same-chunk rule adopted (2026-07-30)

**Measurement:** results/aggregation_provenance/run_20260730T095723Z/, commit b1993d07ad3f6422df9e93c7649ca8d9eedd0f5a, zero-shot MoritzLaurer/mDeBERTa-v3-base-mnli-xnli, thresholds tau=0.5, tau_e=0.9, alpha=0, k=10, retrieval cap applied via select_top_k_chunks as in pipeline.py. Instrument: scripts/measure_aggregation_provenance.py, which calls the production build_claims_chunks_matrix, build_coverage_matrix, score_claim and compute_scoring_result unmodified. 11 claims across SE-028 and GN-040, post-glossary claims arrays from the D102 artifacts.

**Root cause of F3.** engine.py:105-111 computes max_entailment and max_contradiction as two independent max() reductions over a claim's chunk row. Nothing constrains them to the same chunk, and a verification pass confirmed this choice is not a registered decision anywhere: D21's full text lives in InterviewIQ_Pipeline_Docs_v2.0.md, which does not exist in this repository; D42(c) raises an aggregation-composition concern about Coverage only; D43 states only that in the real path the maxima are taken "over a Claims x Chunks matrix"; D45 concerns the merge rule. The provenance of max_c was an undocumented implementation default.

**Measured consequence: max_e and max_c came from different chunks in 11 of 11 claims — never once from the same chunk.** For a correct claim the entailing chunk's contradiction probability is near zero (SE-028 claims 0,1,2,3,5: 0.000167 to 0.001458) while the independent maximum sits at 0.968 to 0.999 on an unrelated chunk in the same document. The independent maximum is therefore saturated for essentially every claim and carries no discriminative information; the verdict is decided by max_e alone, and max_c functions only as a trap for any claim falling below tau_e.

**The specific failure.** SE-028 claim 4 ("تحسن وتنضف Code دون تغيير سلوكه"), a factually correct claim, has max_e = 0.882708 — 0.017 below tau_e — and max_c_independent = 0.999570 from a different chunk, yielding CONTRADICTED at -0.9996: the maximum possible penalty, identical to what a flatly false claim receives. Read from the entailing chunk instead, its contradiction probability is 0.093219 and the verdict becomes NEUTRAL at 0.0. This is the same failure class as F3 (D88/D94/D104), which previously manifested on SE-028 claim 0; the class is now explained rather than merely recorded.

**The fix does not weaken error detection — measured, not assumed.** GN-040 claim 2 carries the deliberately injected numeric error and has max_e = 0.003683 with c_at_argmax_e = 0.995033, so it remains CONTRADICTED at -0.9950 under the same-chunk rule. The mechanism is principled: for a genuinely wrong claim the topically closest chunk is the one that contradicts it, so the contradiction signal read from that chunk is high; for a correct claim it is near zero. Question-level effect: SE-028 Precision 0.666738 to 0.833333 and score 55.863729 to 60.970001; GN-040 Precision 0.600187 to 0.600993 and score 2.289965 to 2.290023, i.e. unchanged. The separation between the correct and the deliberately wrong answer widens. The Coverage channel is unaffected by construction, since it uses max_entailment_for_keypoint only and never consults contradiction; the measured Coverage values are identical under both aggregations.

**Decision:** the aggregation rule is CHANGED. max_c for a claim is read from the chunk that produced max_e, not maximised independently. This replaces an undocumented implementation default with a registered rule, supported by the measurement above. Registered as a decision rather than a bug fix because it alters scoring semantics: contradiction is now evidence-aligned, defined as "does the chunk that best supports this claim also contradict it", rather than "does any retrieved chunk contradict it".

**Correction to D108 (registered, not silently amended):** D108 stated the glossary's effect as negligible on the strength of a +0.004 change in mean Coverage. That finding stands for the Coverage channel only. On the Precision channel the effect is decisive, as the D102 artifacts show: SE-028 claim 0 rose from max_e = 0.232 (CONTRADICTED, D94) to 0.9899 (VERIFIED) with no change other than التست→Test and الكود→Code, and the Precision inversion recorded in D94 and D104 — where the wrong answer scored 0.800 against the correct answer's 0.501 — is gone in v3 (0.600 wrong versus 0.667 correct). GN-040's deliberately wrong claim also moved from NEUTRAL, i.e. unpenalised, to CONTRADICTED at -0.999. D108's wording generalised a Coverage-channel measurement to the layer as a whole; that generalisation is withdrawn here. The glossary is a Precision-channel fix with no measured Coverage-channel benefit.

**Limits of this measurement, stated:** n = 2 questions and 11 claims. Directional, not statistical. The pattern is unanimous (11 of 11) and the mechanism is explained, but broader validation on the 25 O9 questions is a registered follow-up and requires no LLM calls, since the claims already exist in the D107 artifacts.

**Consequences requiring work before the thesis numbers are final:**
- Implementation of the rule change in nli/engine.py or at its call sites, with the existing test suite re-run.
- Re-derivation of every Precision figure produced under the old rule: the pipeline demo artifacts (SE-028, GN-040) and, if cited, D88's two-arm matrices. Two decomposition calls only.
- D89-b's Precision-channel baseline, if that experiment is ever run, must use the new rule.
- The D107/D108 Coverage baseline (zero_shot_final = 0.2928) is unaffected and stands.

**Open items after this entry:** F3 explained and pending the implementation above; low Coverage (D108) still unexplained and now the dominant weakness; broader validation of this rule on the O9 sample; T1, F4-F8, G4, G1, Q2, Q7, G3, Q10.

## D111 — بيت reclassified KEEP to AMBIGUOUS: measured cost of the D98 adjudication exceeded its registered estimate (2026-07-30)

**Trigger — measurement, not preference.** D98 adjudicated near-homograph forms including بيت as KEEP, and D98 named the cost explicitly and accepted it: a form may be substituted even where it carries its other sense. D104 (T1) then observed the cost degrading fixture content in SG-11. The D110 re-run of GN-040 (results/pipeline_demo/GN-040_v4.json, decomposition openai/gpt-oss-120b, same-chunk contradiction rule active) measures the cost as materially larger than the registered estimate: every occurrence of البيت was substituted to bit, including the four claims where the speaker meant Byte, producing claim 2 "bit هو مجموعة من 16 bit" — a self-contradictory sentence — and, more seriously, claims 3 and 4 ("bit هو الوحدة الاساسية التي تقيس حجم البيانات" and "...عنوان الذاكرة") which are factually WRONG as written, since these are properties of the Byte and not the bit, yet were scored VERIFIED at max_e 0.998637 and 0.998755 against GN040-C02. Three of GN-040's five claims are factually wrong and only one was penalised. The substitution made wrong claims resemble correct ones closely enough for the NLI model to entail them, which is subject-blindness of exactly the kind D89 registered as the cost of dropping the adapter — here amplified by the glossary rather than mitigated by it.

**Decision:** the form بيت is reclassified from KEEP to AMBIGUOUS, so it is left in Arabic rather than substituted. Unambiguous forms (بت for bit, بايت for Byte) are unaffected and continue to substitute. No other glossary record is changed and no code is changed: this is a data-level reclassification of one record, using the AMBIGUOUS mechanism already built and registered in D98.

**Rationale for reclassifying rather than attempting disambiguation:** resolving بيت to bit or Byte per occurrence requires reading the surrounding claim's semantics, which is the class of judgment D97 removed from the LLM precisely because it was unreliable. Leaving the form in Arabic is lossy but honest: the NLI model then sees the candidate's actual wording rather than a confidently wrong Latin rendering.
