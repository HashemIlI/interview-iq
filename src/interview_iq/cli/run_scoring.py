...
def run_scoring(
    ...
    pipeline for one document's claims. model/tokenizer/chunk_embedder are
    always injected by the caller — this function never constructs a
    production model itself (see main() for the real-model wiring), so it is
    directly usable from CPU smoke tests with a tiny injected model.
    ...
    for claim in claims:
        capped = select_top_k_chunks(claim.text, document.chunks, k=cfg.k, embedder=chunk_embedder).chunks
        ...
        # (evidence-aligned), not maximised independently over the whole
        # chunk set -- see ClaimsChunksMatrix.best_evidence.
